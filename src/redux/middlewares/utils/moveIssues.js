  ...
  return githubClient.getOcto().then(({repos}) => {
    return Promise.all(cards.map(card => {
      if (label) {
        ...
        return repos(card.repoOwner, card.repoName).issues(card.issue.number).update({labels: labelNames});
      } else if (milestone) {
        return repos(card.repoOwner, card.repoName).milestones.fetchAll().then((milestones) => {
          ...
          if (matchingMilestone) {
            return repos(card.repoOwner, card.repoName).issues(card.issue.number).update({milestone: matchingMilestone.number});
          } else {
            alert(`It seems the target repository (${card.repoOwner}/${card.repoName}) does not have a matching milestone ${milestone.title} to move the Issue(s) to. Please create the milestone manually for now`);
          }
        });
      }
    }));
