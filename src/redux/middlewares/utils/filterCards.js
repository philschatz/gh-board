...
function matchesRepoInfo(repoInfos, card) {
  return repoInfos.some(({repoOwner, repoName}) => {
    return repoOwner === card.repoOwner &&
...
export function filterCard(card, filter, repoInfos, includedTagNames, excludedTagNames, includedMilestoneTitles, excludedMilestoneTitles) {
  const {milestoneTitles, userName, states, types, columnRegExp, tagNames, columnLabels} = filter;
  if (!includedTagNames) {
  ...
  if (!excludedMilestoneTitles) {
    excludedMilestoneTitles = milestoneTitles.filter((m) => m[0] === '-').map((m) => m.substring(1));
  }
  const {issue} = card;
...
export default function filterCards(cards, filter, repoInfos) {
  ...
  const excludedTagNames = tagNames.filter((t) => t[0] === '-').map((t) => t.substring(1));

  const includedMilestoneTitles = milestoneTitles.filter((m) => m[0] !== '-');
  const excludedMilestoneTitles = milestoneTitles.filter((m) => m[0] === '-').map((m) => m.substring(1));

  return cards.filter((card) => {
    return filterCard(card, filter, repoInfos, includedTagNames, excludedTagNames, includedMilestoneTitles, excludedMilestoneTitles);
  });
}
