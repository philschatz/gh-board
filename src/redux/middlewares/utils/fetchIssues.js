...
function _fetchLastSeenUpdatesForRepo(githubClient, repoOwner, repoName, lastSeenAt, isPrivate, didLabelsChange) {
  const opts = {
    per_page: 100,
    sort: 'updated',
...
function _fetchUpdatesForRepo(githubClient, repo) {
  ...
  return Promise.all([
    ...
    return Database.fetchCards(emptyFilter(), [{repoOwner, repoName}]).then((cards) => {
...
export default function fetchIssues (githubClient, filter, repoInfos) {
  ...
  return githubClient.getOcto().then(({users, orgs, repos}) => {
    return Promise.all(repoInfos.map(({repoOwner, repoName}) => {
      if (repoName === '*') {
        ...
        if (githubClient.canCacheLots()) {
          ...
          fetchAllRepos = users(repoOwner).fetch()
            .then(({type}) => {
    ...
    const repos = _.filter(repoAndCards.map(({repository}) => repository), (v) => !!v); // if the lastSeenAt did not change then repository field will be missing
    const cards = _.flatten(repoAndCards.map(({cards}) => cards), true /*shallow*/);
    // didLabelsChange is true if at least one of the repos labels changed
    const didLabelsChange = _.flatten(repoAndCards.map(({didLabelsChange}) => didLabelsChange), true /*shallow*/).indexOf(true) >= 0;
